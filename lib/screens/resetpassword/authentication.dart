...
import 'package:parking_finder_app_provider/screens/resetpassword/new_resetpassword.dart';
...
class AuthenticationResetPasswordState
  ...
  Future<String?> registerProvider() async {
    Map data = {'Email': widget.email, 'OTP': otp};
    String body = json.encode(data);

    final url = Uri.parse('${dotenv.env['HOST']}/provider/verify_otp_forgot');
  ...
  Future<void> resendOTP() async {
    ...
    final url = Uri.parse('${dotenv.env['HOST']}/provider/resend_forgot_otp');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // การส่งไม่สำเร็จ
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('ไม่สามารถส่ง OTP ใหม่ได้')),
        );
      }
    } catch (e) {
      // การเชื่อมต่อมีปัญหา
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('เกิดข้อผิดพลาดในการติดต่อกับเซิร์ฟเวอร์')),
      );
    }
  }
  ...
  Future<void> handleVerify() async {
    ...
    if (result != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('เกิดข้อผิดพลาดในการยืนยัน')),
      );
    }
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: <Widget>[
                ...
                CupertinoButton(
                  padding:
                      const EdgeInsets.symmetric(vertical: 13, horizontal: 160),
