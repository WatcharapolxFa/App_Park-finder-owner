...
class NotificationService {
  ...
  NotificationService({
    ...
    socket.on(
        ...
        (data) => {
              ...
              // onNotificationReceived(Notifications.fromJson(data))
            });
  ...
  Future<List<Notifications>> retrieveNotificationLog() async {
    final url = Uri.parse('${dotenv.env['HOST']}/provider/notification_list');

    // ดึง token จาก storage
    String? token = await storage.read(key: 'accessToken');
    List<Notifications> notificationsLists = [];
    ...
    if (response.statusCode == 200) {
      ...
      if (data['data'] != null) {
        ...
        final List<Notifications> retrievedNotification =
            notificationLogs.map((noti) {
